var uni_mm_proc=(function(){
  //  'use strict';
    ...
    function convert_Zaw_Gyi(txt)
    {
      return convertfont(uni2zg,txt);
    };
    function convert_MM_UNI(txt)
    {
      return convertfont(zg2uni,txt);
    };
    function convertfont(conv_json,txt)
    {
      var max_loop = conv_json.length;

      for(var i=0; i < max_loop; i++) {
        var data = conv_json[i];
        var from = data["from"];
        var to = data["to"];
        var from_regex = new RegExp(from,"g");
        txt = txt.replace(from_regex,to);
      }
      return txt;
    };
    ...
    if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
      ...
      if (typeof define === 'function' && define.amd) {
        ...
        window.uni_mm_proc = uni_mm_proc;
        return uni_mm_proc;
      }
    }
